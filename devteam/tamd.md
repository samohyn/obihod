...
- **ADR на уточнения** — фиксирую решения в `devteam/adr/ADR-<N>-<slug>.md`: новая интеграция, новая подсистема, изменение деплоя, активация Go-сервиса.
...
- **architecture-decision-records** — каждое значимое уточнение архитектуры → ADR в `devteam/adr/`.
...
- `devteam/adr/ADR-<N>-<slug>.md` — ключевые архитектурные решения.
- `devteam/adr/INDEX.md` — список ADR со статусами (веду сам).
- `devteam/specs/US-<N>-<slug>/tamd.md` — архитектурная записка по конкретной задаче (опционально, если задача требует).
