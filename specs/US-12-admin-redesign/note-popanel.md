...
## Decision log 2026-04-30 night · Wave 9 launch + closed (autonomous-mode 2)

### D-2026-04-30-09 · 3 issues после merge PR #109

**Контекст:** оператор открыл prod /admin (после merge W8) и зафиксировал 3 новых проблемы:
1. **Favicon кривой** — выглядит как обрывок ico
2. **Тёмная тема внутри панели** — на edit view inputs/tabs/status/right-sidebar тёмные
3. **Кривой лого в crumbs** — BrandIcon выглядит как обрезанный полу-circle

Оператор дал mandate: pristal'no сверить admin vs §12 макеты + fix.

**Skill activation (popanel iron rule):** blueprint + product-capability — снова через autonomous-mode (повтор pattern D-04..D-08).

### D-2026-04-30-10 · Root cause analysis (3 fix points)

**Verified через Playwright DevTools eval + Payload source чтение:**

1. **W9.1 force-light depth gap:** Payload в `data-theme='dark'` инвертирует `--theme-elevation-*` через `var(--color-base-N)` (см. `node_modules/@payloadcms/ui/dist/scss/colors.scss` lines 190-212). Наш W1 override only `--brand-obihod-*` + selected `--color-base-*` (0/50/100/150/200/900/1000), **не сами** `--theme-elevation-*` напрямую. При prefers-color-scheme: dark → tab/input/status backgrounds становятся ink. **Это и есть «тёмная тема внутри панели».**
2. **W9.2 BrandIcon size:** SVG 32×32 в `.step-nav` slot высотой 20px → vertical clipping = «кривой лого».
3. **W9.3 favicon mismatch:** `payload.config.ts` admin.meta.icons был `type: 'image/png'` для url `/favicon.ico` — браузеры могут отвергать как corrupt MIME.

### D-2026-04-30-11 · 3 fixes (autonomous-mode dev)

| Fix | Файл | Изменение |
|---|---|---|
| W9.1 | `site/app/(payload)/custom.scss` | `html[data-theme='dark']` block extended +25 lines: direct override `--theme-elevation-0..1000` (cream-to-ink scale) + `--theme-bg/--theme-input-bg/--theme-text` явно |
| W9.2 | `site/components/admin/BrandIcon.tsx` | 32×32 → 20×20 SVG (viewBox + cx/cy/r/font scaled) |
| W9.3 | `site/payload.config.ts` | admin.meta.icons: `type: 'image/x-icon'` + url `/favicon.ico` (correct) + PNG fallback `/icon.png` |
| W9.qa | `site/tests/e2e/admin-design-compliance.spec.ts` | +3 W9 e2e tests: force-dark theme tokens stay light + BrandIcon 20×20 + favicon type/url consistency |

### D-2026-04-30-12 · Verification (all green)

- ✅ `pnpm type-check` 0 errors
- ✅ `pnpm lint` 0 errors
- ✅ `pnpm format:check` clean
- ✅ `pnpm test:e2e --grep "Wave 8|Wave 9"` — **5/5 passing** в 6.7s (W8: 2/2, W9: 3/3)
- ✅ Playwright local-verify dark mode forced: edit-view (services/5) — все inputs/tabs/sidebar light, BrandIcon 20×20, favicon DOM links корректные
- ✅ Light mode regression: `/admin`, `/admin/collections/services`, edit view — без визуальных разрывов

**Screenshots evidence:**
- `screen/wave9-dashboard-light.png` — dashboard light baseline
- `screen/wave9-edit-view-dark-fixed.png` — edit view forced dark, всё light
- `screen/wave9-edit-view-dark-final.png` — edit view forced dark final state
- `screen/wave9-list-view-light.png` — list view light baseline
- `screen/wave9-list-view-dark-fixed.png` — list view forced dark, всё light

**Pristal'no audit findings (deferred, не блокеры W9):**

- **W1 `.payload__app` префикс cleanup** — большинство W1 правил (btn-primary, secondary, inputs focus, status pills, tabs) prefixed с `.payload__app` ancestor, который **отсутствует** на admin shell. Эти rules `dead code`, но визуально admin выглядит правильно благодаря `--theme-elevation-*` cascade (W9.1 fix). Cleanup не нужен срочно. → backlog `PANEL-CSS-PREFIX-CLEANUP`.
- **List "Создать" button styling** — Payload использует `.btn--style-pill` (#d6d0bf line-hover bg). brand-guide §12.4.1 primary CTA должен быть янтарным. → backlog `PANEL-LIST-CREATE-AMBER`.

### D-2026-04-30-13 · Hand-off log

```
21:00 operator → popanel: 3 issues + pristal'no audit mandate
21:00 popanel: blueprint + product-capability skills activated
21:30 popanel → fe-panel: 3 fixes
21:50 popanel local-verify: 5 e2e tests + 5 screenshots evidence
22:15 popanel → do: green CI
22:30 popanel → release: готов к gate (PR #110 будет создан)
```
