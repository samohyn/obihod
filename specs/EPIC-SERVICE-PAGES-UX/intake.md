...
- 2026-05-11 14:35 MSK · cw+dev → po: **C5 wave B done (vyvoz-musora pilot — real content).** Wave A провалился template-fallback'ом (placeholder cards «черновик, в работе» — операторская фигня). Здесь контент написан вручную (Claude в задаче, БЕЗ ANTHROPIC_API_KEY), TOV brand-guide v2.6 §tov Caregiver+Ruler, конкретные цифры.
  - **3 новых блока** на Service id=3 (direct SQL на prod, idempotent — `scripts/fill-vyvoz-musora-content.sql`): (a) `calculator-placeholder` _order=12 с реальным текстом «Точный расчёт за 10 минут — пришлите три фото объекта» + body про 3 фото + CTA `/foto-smeta/?service=vyvoz-musora` (вместо пустого блока / TODO-жаргона); (b) `pricing-table` _order=13, variant=tiers, 3 тарифа Эконом 12 800 / Стандарт 14 200 (highlighted, badge «Выбирают чаще») / Премиум 14 800 ₽, по 4 features каждый, 8/20/27 м³; (c) `process-steps` _order=14, layout=vertical, 6 шагов заявка→расчёт→подача контейнера→погрузка→вывоз и утилизация→акт + закрывающие, per-step eta.
  - **Markdown-bug fixed**: `text-content` body был Lexical-объектом с raw `## Как мы работаем\n\n…` (10 H2-секций) внутри ОДНОГО paragraph node → RichTextRenderer рендерил литерально. Перезаписан чистым markdown-string (jsonb scalar); `TextContent.tsx` для строки использует `renderMarkdown` → `## ` → `<h3>`. Раздел `## Как мы работаем` убран из текста — его покрывает новый `process-steps` блок (фикс дубля + бага).
  - **Migration `20260511_120000_services_blocks_v26_extension`** ({ts,up.sql,down.sql}): `services_blocks_pricing_table` (+`_tiers`, `_tiers_features`) + `services_blocks_process_steps` (+`_steps`) + `_services_v_*` mirror (10 таблиц, 4 ENUM, idempotent: DO-guards для TYPE/CONSTRAINT, IF NOT EXISTS для TABLE/INDEX). Зеркало того, что ServiceDistricts получил в C2.6. **Applied на prod** (scp → `sudo -u postgres psql -v ON_ERROR_STOP=1 -f` → registered в `payload_migrations` → ownership→obikhod на 10 таблиц + 4 типа + serial sequences). Backup ДО: `Prod backup (pg_dump)` run **#25667115074** SUCCESS.
  - **Code changes (working tree, НЕ pushed — constraint):** `site/collections/Services.ts` blockReferences 11→13 (+PricingTable +ProcessSteps); `site/lib/master-template/getBlocksForLayer.ts` SLUG_TO_SECTION +`pricing-table`→`pricing-block` +`process-steps`→`process` (sync с уже-задеплоенным `master-template.ts` где aliases уже есть); 3 migration files; `site/scripts/fill-vyvoz-musora-content.{ts,sql}`.
  - **Verification:** `pnpm type-check` 0 errors / `pnpm exec eslint` 0 errors на изменённых / `pnpm exec prettier --check` clean / `pnpm test:unit` **59/59 PASS** (resolver alias не ломает unknown-drop тесты — `mystery-block` всё ещё drop).
  - **`use_template_v2` = FALSE** для id=3 (решение): deployed resolver НЕ знает `pricing-table`/`process-steps` aliases (только мой working-tree fix их добавляет) → при templateV2=true дропнул бы реальные блоки и нарисовал placeholder'ы «Прайс-лист добавляется» / «Шаги работы добавляются». При templateV2=false → identity passthrough, deployed `BlockRenderer` рендерит все 14 блоков как есть, 0 placeholders. **СЛЕДУЮЩЕЕ для po:** commit+deploy code changes (resolver alias + Services.ts blockReferences + migration files) → потом `UPDATE services SET use_template_v2=true WHERE id=3` → получим master-template reorder + pricing/process visible. До deploy: pricing-table/process-steps rows в DB но НЕ рендерятся (deployed Payload config их не знает в blockReferences → afterRead игнорит).
  - **Pre-existing landmine (out of scope, не трогал):** 2 `cta-banner` блока на doc 3 (order 4+9) → master-template-gate бросит `DUPLICATE_SECTION` при re-save в admin (resolver dedup'ит → keep first).
  - **Smoke prod `/vyvoz-musora/`:** HTTP **200** (0.69s) · `data-placeholder` count **0** · raw `## ` leak **0** · TODO-жаргон («по брифу»/«в работе»/«добавляется»/«sustained») **0** · calculator real copy present · 7 JSON-LD nodes · H2-list реальный (`Что вывозим` / `Как мы работаем` (process-steps h2) / `Получите смету за 10 минут` / `Частые вопросы по вывозу мусора` / `Также берём на себя` / `Точный расчёт за 10 минут…`) + h3-list реальный (`Что входит в цену` / `Какой мусор берём и какой нет` / `Сроки и регламент` / `Почему одной бригадой дешевле` / `Документы, СРО и страховка` / `Районы и пояс выезда` / `Какие фото нужны для сметы` / `Оплата и договор`) · other pillars (arboristika/chistka-krysh/demontazh) HTTP 200. **No git commit/push** (intake constraint).
  - **Готово к** design review + (после po deploy) A/B re-enable.
