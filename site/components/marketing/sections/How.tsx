...
/**
 * How — секция §03 (5 шагов: заявка → выезд → смета → работа → оплата+гарантия).
 * Source: newui/homepage-classic.html.
 * Phase 3: контент из Homepage.steps[] с graceful fallback. Иконки остаются inline
 * (по позиции 0-4) — admin может изменить только title/sla/description.
 */
...
const FALLBACK_STEPS = [
  {
    ...
    description: 'Отвечаем на звонок или сообщение в WhatsApp / Telegram. Уточняем задачу, район.',
  },
  {
    title: 'Выезд',
...
const STEP_ICONS = [
  ...
  // 3: дом + флаг
  <path key="3" d="M3 21h18M5 21V10l7-5 7 5v11M10 21v-6h4v6" />,
  // 4: галочка в круге
  <g key="4">
...
export function How({ data }: { data?: HomepageGlobal }) {
    ...
    <section className="hp-section">
      <div className="wrap">
        ...
        <div className="hp-how">
          {steps.map((s, i) => (
            <div className="hp-how-card" key={i}>
              <svg
                className="ic"
                viewBox="0 0 24 24"
                fill="none"
              ...
              <h4 className="t">{s.title}</h4>
              <p className="sla">{s.sla}</p>
