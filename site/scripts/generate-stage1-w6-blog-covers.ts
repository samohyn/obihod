/**
 * Stage 1 W6 Track B Run 2 — batch генерация 5 blog cover images.
 *
 * Читает `site/public/uploads/stage1-w6/prompts-batch.json`, прогоняет каждый
 * item через fal.ai, скачивает JPEG в outFile. Идемпотентен: если файл уже
 * есть — skip (use --force чтобы перегенерировать).
 *
 * Запуск:
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w6-blog-covers.ts
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w6-blog-covers.ts --force
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w6-blog-covers.ts --only=blog-4-v-1-cover
 *
 * art consistency: все prompts через `blogCoverPrompt({ visualConcept })`
 * с `no humans, no figures, no silhouettes` guard (см. prompts.ts §Stage 1 W5/W6).
 */
...
const BATCH_PATH = resolve(REPO_ROOT, 'site/public/uploads/stage1-w6/prompts-batch.json')
...
async function main(): Promise<void> {
  ...
  console.log(`Stage 1 W6 blog-cover generation: ${items.length} item(s), force=${force}`)
