/**
 * Stage 1 W4 Track B Run 1 — batch генерация 5 hero images.
 *
 * Читает `site/public/uploads/stage1-w4/prompts-batch.json`, прогоняет каждый
 * item через fal.ai, скачивает PNG в outFile. Идемпотентен: если файл уже
 * есть — skip (use --force чтобы перегенерировать).
 *
 * Запуск:
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w4-heroes.ts
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w4-heroes.ts --force
 *   pnpm tsx --env-file=.env.local scripts/generate-stage1-w4-heroes.ts --only=pillar-vyvoz-musora-hero
 *
 * Требует FAL_KEY в site/.env.local. Без ключа — exit 1, fail-fast.
 *
 * art consistency: все prompts через `pillarHeroPrompt` / `uspFotoSmetaPrompt`
 * с `no humans, no figures, no silhouettes` guard (см. prompts.ts §Stage 1 W4).
 */
...
const BATCH_PATH = resolve(REPO_ROOT, 'site/public/uploads/stage1-w4/prompts-batch.json')
...
// fal flux/schnell часто возвращает JPEG-байты по `.png` URL/extension.
// Подменяем расширение на основе magic bytes, чтобы файл на диске был
// truthful относительно MIME (важно для Payload Media: mimeTypes white-list).
...
async function processItem(item: BatchItem, force: boolean): Promise<void> {
  ...
  // Идемпотентность по любому known image-расширению.
...
async function main(): Promise<void> {
  ...
  console.log(`Stage 1 W4 hero generation: ${items.length} item(s), force=${force}`)
