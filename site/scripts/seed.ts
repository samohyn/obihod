...
const SERVICES: ServiceSeed[] = [
  ...
  },
  {
    ...
    metaDescription:
      'Чистим крыши от снега и наледи в Москве и МО по фикс-цене за м². Выезд бригады 24/7, промальп и автовышка, страховка ГО 10 млн ₽, смета по фото в Telegram за 10 минут.',
    intro:
  ...
  },
  {
    ...
    metaDescription:
      'Вывозим мусор в Москве и МО по фикс-цене за м³. Строительный, порубочные остатки, хлам с участка. Погрузка и уборка — входит. Смета в Telegram, MAX, WhatsApp за 10 минут.',
    intro:
  ...
  },
  {
    ...
    metaDescription:
      'Сносим дачные дома, бани, сараи, хозблоки в Москве и МО по фикс-цене за объект. Вывоз мусора включён, порубочный билет за наш счёт. Смета по фото в Telegram, MAX, WhatsApp.',
    intro:
...
async function main() {
    ...
    if (!chrome?.contacts?.phoneE164) {
      await payload.updateGlobal({
        ...
        data: {
          ...
          requisites: {
            // legalName оставляем пустым до регистрации юрлица (memory: project_inn_temp).
            // taxId — временный (СПб), оператор заменит после регистрации ООО «Обиход».
            taxId: '7847729123',
          },
          social: [],
        } as never,
  ...
  for (const d of DISTRICTS) {
    try {
      ...
      const metaDescription = `Все услуги Обихода ${d.namePrepositional}: спил деревьев, чистка крыш, вывоз мусора, демонтаж. Фикс-цена за объект, смета по фото за 10 минут в Telegram, MAX или WhatsApp.`
      const created = await payload.create({
        collection: 'districts',
        data: {
          slug: d.slug,
