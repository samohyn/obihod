...
Артефакт: `team/specs/US-<N>-<slug>/sa.md`. Возврат `→ po` → оператор
утверждает.
...
`team/specs/US-<N>-<slug>/qa.md` (pass или bug report с воспроизведением).
...
PR или `team/specs/US-<N>-<slug>/cr.md`.
...
**po** пишет `team/release-notes/US-<N>-<slug>.md` (шаблон в §8). **do**
...
зеркалируется как Issue в Linear: workspace **`samohyn`**, team/project key
**`OBI`**.

> **Нумерация — единая (с 2026-04-27).** Primary identifier — `OBI-<N>`
> (Linear-автонумерация). `US-<N>` больше не используется в title Linear-issue —
> переходит в **label `epic:us-<N>`** для группировки. Папки `team/specs/US-<N>-<slug>/`
> в репо сохраняются как исторические — обновлять их можно, переименовывать не
> требуется. В шапке `intake.md` / `ba.md` / `sa.md` фиксируется только
> `**Linear Issue:** OBI-<N>` без `US-<N>`-дублирования.
>
> **Sub-tasks** (например, OBI-19 как «US-12.4 SCSS override») — через нативный
> Linear `parentId` (parent = OBI-24 / US-12). В title sub-task — без `US-<X>.<Y>:`
> префикса, только семантический заголовок.
...
| Артефакты (intake, ba, sa, qa, cr, out) | `team/specs/US-<N>-<slug>/` в репо | соответствующие роли |
| ADR | `team/adr/` в репо | `tamd` |
| Research | `team/specs/US-<N>-<slug>/re.md` или `team/research/` в репо | `re` |
| Release notes | `team/release-notes/` в репо | `po` |
...
> **2026-04-27 миграция:** label-группы `role`, `phase`, `segment`, `epic` —
> hierarchical Linear groups (см. §7.5.3). Внутри группы можно одну метку (для
> phase) или несколько (для role). Имена `phase/ba`, `phase/sa`, `phase/cr`
> переименованы в **`phase/spec`** и **`phase/review`** из-за коллизии с
> `role/ba`, `role/sa`, `role/cr` (Linear не позволяет одинаковые имена в одной
> teamspace).

| Фаза [WORKFLOW §3] | Linear state | Label `phase/*` | Label `role/*` (пример) |
|---------------------|--------------|------------|---------------------|
| 1. Intake | Backlog | `phase/intake` | `role/in` |
| 2. BA + 4. SA (объединены) | In Progress | `phase/spec` | `role/ba`, `role/sa`, `role/re` |
| 3. PO planning | In Progress | `phase/spec` | `role/po` |
| 5. Architecture | In Progress | `phase/spec` | `role/tamd` (+ `role/dba` при данных) |
...
| 9. Code Review | In Progress | `phase/review` | `role/cr` |
| 10. Acceptance | In Progress | `phase/release` | `role/out` |
| 11. Released | Done | `phase/release` | `role/po` (закрытие) |
...
### 7.5.3. Labels (единый каталог, обновлён 2026-04-27)

Linear-labels организованы в **5 hierarchical групп** + плоские type-labels.
Внутри группы — одна метка (для `phase`, `segment`, `epic`) или несколько (для
`role`).

**Type-labels** (плоские, тип задачи): `Feature` · `Bug` · `Research` · `Ops` ·
`Content` · `Design Refresh` · `Improvement` · `Design System` · `Tech Debt`.

**Priority** (плоские, MoSCoW / приоритет): `P0` (blocker) · `P1` (high) · `P2`
(normal) · `P3` (low).

**Group `role`** (28 кодов команды, **одна** ведущая роль на issue —
Linear делает группы mutually exclusive):
`in` · `ba` · `sa` · `po` · `tamd` · `art` · `ux` · `ui` · `lp` · `fe1` · `fe2` ·
`be1` · `be2` · `be3` · `be4` · `cr` · `qa1` · `qa2` · `cw` · `seo1` · `seo2` ·
`aemd` · `da` · `pa` · `do` · `re` · `dba` · `out`. Дополнительные роли —
в Description секции «Команда».

**Group `phase`** (одна метка одновременно): `intake` · `spec` (объединение
ba+sa+po+tamd) · `design` · `dev` · `qa` · `review` (cr) · `release`.

**Group `segment`** (одна метка): `b2c` · `b2b` · `cross`.

**Group `epic`** (одна или несколько меток для группировки): `us-3` · `us-4` ·
`us-5` · `us-6` · `us-7` · `us-8` · `us-9` · `us-10` · `us-11` · `us-12` ·
`us-13` · `us-14` · `us-15` · `programmatic` (для не-эпичных follow-up — без
метки epic).
...
1. **Создание.** `in` создаёт Issue при оформлении `intake.md`. Title =
   `<семантический заголовок>` (без `US-<N>:` префикса — единая нумерация
   через OBI-N как primary id с 2026-04-27). State = Backlog. **Assignee = оператор.**
   Labels (минимум 6): priority + type + `epic/us-<N>` (если эпик) + `role/in` +
   `phase/intake` + `segment/<b2c|b2b|cross>`.
...
### 7.5.5. Шаблон description Issue (обновлён 2026-04-27)
...
**Тип:** <Feature / Bug / Research / Ops / Content / Design Refresh / Improvement>
...
- SA: `team/specs/US-<N>-<slug>/sa.md`
- ADR (если есть): `team/adr/ADR-<M>-<slug>.md`
- QA report: `team/specs/US-<N>-<slug>/qa.md`
- Release note (после релиза): `team/release-notes/US-<N>-<slug>.md`
...
Файл: `team/release-notes/US-<N>-<slug>.md`. Владелец — **po**.
...
**Выходы:** `team/specs/US-<N>-<slug>/out.md`:
...
- Новые роли — добавить в [PROJECT_CONTEXT.md](PROJECT_CONTEXT.md) +
  ...
  `team/` + обновить §1 здесь + добавить `role:<code>` в §7.5.3.
