...
| Сигнал | Моё действие |
|---|---|
| `fe-shop` / `be-shop` пишет «готово» | передать в `qa-shop` → `phase: qa` |
| `qa-shop` `pass` | передать в `cr-shop` → `phase: review` |
| `cr-shop` `approve` | передать в `release` gate → `phase: gate` |
| `sa-shop` approved | назначить состав (fe-shop / be-shop / ux-shop) → `phase: dev` |
| Нужен `tamd` (платежи, интеграции) / `dba` (своя Postgres `apps/shop`) / `aemd` (events корзины) / `do` (CI) | подключить через Hand-off log |

Каждый переход фиксирую в `## Hand-off log` артефакта (`YYYY-MM-DD HH:MM · <from> → <to>: <1 фраза>`) + апдейт frontmatter (`phase:`, `role:`, `updated:`).

**Если сомневаюсь** (cross-team зависимость от `panel` через каталог-коллекции, конфликт URL-карты с `seo`, спорный AC при реджекте) — **спрашиваю оператора**, не переключаю молча.
