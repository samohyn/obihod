...
## Где мы сейчас (2026-05-11 ~17:05 MSK) — EPIC-SERVICE-PAGES-REDESIGN master-template rollout на 4 pillars (arboristika / chistka-krysh / demontazh / dizain-landshafta)

**cw+dev (autonomous mandate от po):** раскатал паттерн pilot /vyvoz-musora/ на 4 оставшихся pillar.

- 2026-05-11 17:05 MSK · cw+dev → po: **rollout 4 pillars done.** Direct idempotent SQL на prod (DELETE-then-INSERT), backup сделан до batch (`gh workflow run "Prod backup (pg_dump)"` → SUCCESS run 25683666311). Скрипты сохранены в `/tmp/rollout-rollback/fill-pillar-{1,2,4,11}-*.sql` + `text_content_124.json` (бэкап удалённых legacy text-content rows id 1/2/4). **id=1 arboristika** (`service_type=spil`): DELETE text_content, INSERT pricing-table 3 tier (Эконом от 3 500 / Стандарт от 4 800 / Премиум от 6 500 ₽ за дерево), process-steps 7 шагов (заявка→оценщик→согласование→бригада→спил→вывоз остатков→акт), calculator real copy (фото дерева), mini-case inline_facts (Ø80см / 1 день / 6 800 ₽), `use_template_v2=true`. **id=2 chistka-krysh** (`service_type=krysha`): pricing 3 tier (35 / 50 / 70 ₽/м²), process 6 шагов, calc copy, mini-case facts (180 м² / 4 ч / 9 000 ₽), v2=true. **id=4 demontazh** (`service_type=demontazh`): pricing 3 tier (8 000 / 25 000 / 50 000 ₽ объект), process 7 шагов, calc copy, mini-case facts (48 м²→18 м³ / 2 дня / 32 000 ₽), v2=true. **id=11 dizain-landshafta** — БЫЛ БЕЗ blocks[] (seeded 2026-05-10 legacy-style: `intro` + `services_sub_services`×5 + `services_faq_global`×3, рендерился через non-block ветку `[service]/page.tsx`). Чтобы master-template заработал — построил ПОЛНЫЙ набор T2_PILLAR блоков с нуля: breadcrumbs(1, empty items — рендерит null, как у всех v2-pillars) / hero(2) / tldr(3, text из legacy intro + расширен) / services-grid(5)+5 items (из subServices, ссылки `/dizain-landshafta/<sub>/` уже 200) / cta-banner(4) / lead-form(7) / mini-case(8)+facts (Одинцово 12 соток / 3 нед / 380 000 ₽) / faq(10)+4 items (3 из legacy faqGlobal + 1 «сколько стоит») / related-services(11)+3 items (arboristika/demontazh/vyvoz-musora) / calculator(12, `service_type=NULL` — нет enum-value для landshaft, валиден) / pricing-table(13)+3 tier (50 000 / 150 000 / 500 000 ₽ проект) / process-steps(14)+8 шагов (заявка→участок→концепция→3D→согласование→рабочий проект→реализация→гарантия приживаемости). `use_template_v2=true`. Lexical richText для faq.answer + cta-banner.body. `pm2 restart obikhod` ×2 (после pillars 1/2/4 и после id=11). **Smoke (curl + Chrome UA):** все 5 pillars `/arboristika/ /chistka-krysh/ /demontazh/ /dizain-landshafta/ /vyvoz-musora/` → HTTP 200, **0 bad-markers** (`data-placeholder` / `[object Object]` / raw `## ` / `TODO `), pricing-tiers рендерятся (Эконом/Стандарт/Премиум ×2 каждый), «Как мы работаем» ×2, calc copy ×2, 12 CTA-кнопок «Получить смету по фото» на каждом (= как pilot). dizain-landshafta: hero/tldr/grid/pricing/process(шаг 08 «Гарантия приживаемости»)/mini-case(Одинцово)/faq/cta/related/lead-form все рендерятся, legacy-маркер «Что входит в раздел» отсутствует → точно v2-ветка. **SD pages (`use_template_v2=false`):** `/vyvoz-musora/khimki/ /chistka-krysh/odincovo/ /arboristika/ramenskoe/ /demontazh/zhukovskij/` → HTTP 200, legacy не задета. **Sub-pages landshaft:** `/dizain-landshafta/proekt-uchastka/ /dizain-landshafta/gazon-i-poliv/` → 200. **0 pillars crashed / rolled-back.** **Photos = gradient fallback** для всех 4 (нет hero/mini-case Media wire — D2 wave B-E pending budget; у id=11 hero `image_id=NULL`, mini-case `inline_photo_id=NULL`). **Pre-existing quirk замечен (не правил):** `CtaBanner.tsx` читает `block.h2 ?? block.heading`, но Payload-поле `cta-banner.title` → колонка `title` → heading НЕ рендерится ни у одного pillar (включая pilot). cta-banner body + CTA-кнопка рендерятся ОК. **Не делал:** git commit/push (constraint), правки brand-guide/master-template.ts/Services.ts (всё уже задеплоено).

## Где были раньше (2026-05-11 ~16:10 MSK) — EPIC-SERVICE-PAGES-REDESIGN pilot polish done · /vyvoz-musora/ wall-of-text убран + hero/mini-case с фото live

**design+fe (autonomous mandate от po):** 2 quick fix для pilot pillar /vyvoz-musora/.

- 2026-05-11 16:10 MSK · design+fe → po: **pilot polish done.** **A (wall-of-text):** opt.1 — `services_blocks_text_content` row (`_parent_id=3`, heading «Подробно: цены, регламент, документы», body 8071 симв. с raw `##`) **удалён** idempotent SQL (backup `/tmp/pilot-rollback/text_content_id3_backup.json`). text-content presence на T2 = `optional` → resolver не re-fill placeholder'ом. Дублировал process-steps(6)+pricing-table(3 tier)+faq+tldr. **B (photo wire-up):** 22 PNG→WebP (sharp 0.34.5 из site/node_modules; hero 1024px q74 ≤200KB, cases 820-900px q48-72 ≤150KB, process/calc/districts 900px q68); WebP в `assets/services/vyvoz-musora/optimized/`. **Payload Media REST upload НЕ работает** — на prod у единственного user `enable_a_p_i_key=NULL`, `api_key IS NULL` (ключ в `~/.claude/secrets/obikhod-payload.env` мёртвый → 403 «no permission»); tsx нет на prod (Local API недоступен). **Workaround:** 13 WebP положены в `/home/deploy/obikhod/shared/media/` + symlink `current/media → shared/media` (Payload `staticDir:'media'` читает диск динамически, не build-time Next public manifest — `/api/media/file/<f>` отдаёт 200). 13 Media DB-rows вставлены idempotent SQL (id 61-73, `url=/api/media/file/<f>.webp`, `license='proprietary'` + caption-note «license generated-fal-ai» т.к. select-enum не имеет такого option). **Wired:** `services_blocks_hero.image_id=61` (hero-1-desktop.webp), `services_blocks_mini_case.inline_photo_id=63` (case1-after.webp — Одинцово). process/calc/districts WebP забанкованы в Media (id 65-73), но wire негде — у `services_blocks_process_steps_steps`/`calculator_placeholder` нет media-колонок (схему не трогали). **next/image fix:** `<Image>` падал 400 «"url" parameter is not allowed» — Payload отдаёт upload-URL абсолютным (`serverURL`=https://obikhod.ru + `/api/media/file/`), а `next.config.ts` не имел `images.remotePatterns`. Добавил `remotePatterns: [obikhod.ru, www.obikhod.ru, localhost]` — **PR #223** (local edit + type-check 0 / lint 0 errors / prettier clean) + **prod hotfix** (scp local next.config.ts → current/ + pm2 restart; первая попытка inline-patch скрашила сервер «ReferenceError: https is not defined» — single-quote escaping в ssh, откатил из бэкапа, починил через scp; restart count стабилен 35, uptime 4m+). `pm2 restart obikhod` ×2 (после wire + после config). **Smoke `/vyvoz-musora/`:** HTTP 200 · 183KB · 0.3s, hero `<img>` → `/_next/image/?url=...hero-1-desktop.webp` (рендер 576px desktop / 390px mobile, complete), mini-case → `...case1-after.webp` (719px, complete), next/image optimizer 200 (jpeg/avif variants), 0 `data-placeholder`, 0 `[object Object]`, 0 `_placeholder`, 0 raw `##`, text-content heading отсутствует. Other 4 pillars (`use_template_v2=false`) + T3 `/vyvoz-musora/gazel/` → HTTP 200, не задеты. **Screenshots:** `screen/PILOT-2026-05-11/vyvoz-musora-{desktop,mobile}-v2.png` (Playwright 1440×900 / 390×844, scroll-lazy-load + 3-3.5s wait). **Blockers / follow-ups:** (1) `media/` symlink внутри release-dir — `deploy.yml` симлинкает только `.env`, не `media/` → следующий deploy убьёт symlink, файлы в `shared/media/` останутся → нужно devops добавить `ln -sfn $BP/shared/media media` в deploy.yml «Unpack release» step ИЛИ получить рабочий Payload API key + re-upload через REST (правильнее — REST генерит size-variants). (2) `next.config.ts remotePatterns` живёт на prod как hotfix в release-dir → PR #223 надо смержить чтобы пережил deploy. (3) Payload API key мёртв в `~/.claude/secrets/obikhod-payload.env` — оператору пересоздать на /admin user page + обновить secret.

## Где были раньше (2026-05-11 ~15:48 MSK) — EPIC-SERVICE-PAGES-REDESIGN D3-fix done · /vyvoz-musora/ v2 live, 500 пофикшен

**fe+dev (autonomous mandate от po):** hotfix prod-500 после enable `use_template_v2=true` на /vyvoz-musora/.

- 2026-05-11 15:48 MSK · fe+dev → po: **D3-fix done.** Root cause prod-500 (`Error: Objects are not valid as a React child (found: object with keys {id, value})`): 4 D3 wave A блок-рендерера читали Payload `array`-поля напрямую как React child (`<li>{item}</li>` где item = `{id, value}` после afterRead, а не `string`) — `PricingTable.tsx` (`tier.features`), `TldrV2.tsx` (`bullets`), `MiniCaseV2.tsx` (`meta`, `thumbs`=`{id,url}`), `ServiceHero.tsx` (`trust`). Прочие D3 wave A компоненты (usps/kpis/steps/items в TrustBlock/FaqAccordion/DistrictChips/ProcessSteps) типизированы как объекты-rows → не крашат, проверены все. **Fix:** `toStringList()` в `site/components/blocks/types.ts` — defensive normalizer (`string[]` legacy fixtures + Payload `[{id,value}]`/`[{id,url}]` → `string[]`); 4 рендерера зовут её. **Test:** `site/components/blocks/__tests__/array-render-shape.test.tsx` (`renderToStaticMarkup` + `tsx --test`, без новых deps) — рендер каждого блока с Payload `{id,value}` shape → no-crash + контент + нет `[object Object]`; + unit на `toStringList`; подключён в `test:unit`. type-check 0 / test:unit 68/68 pass / lint 0 errors / prettier clean. **PR #222 squash-merged → main `d67a842` → Deploy to Beget SUCCESS.** Content re-insert: `site/scripts/fill-vyvoz-musora-content.sql` (idempotent, через `ssh root@... 'sudo -u postgres psql obikhod -v ON_ERROR_STOP=1'` ← stdin, т.к. `scripts/` не в deploy-артефакте) → pricing-table+3 tiers+12 features, process-steps+6 steps, calculator re-insert, text-content markdown-fix, `use_template_v2=true WHERE id=3`. `pm2 restart obikhod` для ISR flush. **Smoke `/vyvoz-musora/`:** HTTP 200, H1 «Вывоз мусора в Москве и МО — 12 800 ₽…», pricing tiers (Эконом/Стандарт/Премиум 12 800/14 200/14 800 + features), process steps (Заявка / Подача контейнера или Газели / Вывоз и утилизация / «Как мы работаем»), 0 `data-placeholder`, 0 `[object Object]`, 0 error-page. Other pillars (arboristika/chistka-krysh/demontazh/dizain-landshafta — `use_template_v2=false`) → HTTP 200 legacy, не задеты. **Blockers: нет.** D3 redesign live на pilot pillar /vyvoz-musora/.

## Где были раньше (2026-05-11 ~15:05 MSK) — EPIC-SERVICE-PAGES-REDESIGN D2 wave A done (52 photos via fal.ai)

**design (autonomous mandate + operator передал FAL_KEY 2026-05-11):** D2 wave A — генерация фото для pilot pillar /vyvoz-musora/ через fal.ai.

- 2026-05-11 15:05 MSK · design → po: **D2 wave A done**. 52 photos via fal.ai REST API (`fal-ai/nano-banana-pro` — MCP не доступен в сессии → прямой REST через curl/urllib + `SSL_CERT_FILE=$(python3 -c 'import certifi;print(certifi.where())')` для Python 3.14 SSL). Buckets в `assets/services/vyvoz-musora/`: hero 8 (4 desktop + 4 mobile), process 5 (step1-5), mini-case 6 (case1-3 × before/after), districts 30 (cluster mapping industrial-suburb / dacha-suburb / historic-town / greater-moscow-rim — визуально различны, скип-контейнер вписан в сцену а не subject), calculator 3 (stage1-3). Cost ≈ **$2.75** (52 final + 2 case2 retries + 1 smoke = 55 generations × ~$0.05/img) — well under $5 cap. Review: `assets/services/vyvoz-musora/REVIEW.md` — **52/52 ✅ approved**, 0 reject, 1 reroll (case2 before/after — первый проход словил случайных людей → перегенерил с `no people` negative). A/B pilot pick: desktop=H1d (`hero/1-desktop.png`), mobile=H1m (`hero/1-mobile.png`), backup desktop=H4d (golden-hour). Privacy 152-ФЗ: спины/руки/силуэты без лиц (на districts дальние фигуры со спины — в рамках правила). Без логотипов, без overlay-text, документальный репортаж по `foundations/photography.md`. **Caveat для fe (D3):** nano-banana-pro игнорит `image_size` — большинство файлов почти-квадрат ≈3500px, не точные 16:9/4:3 → fe кропит при импорте (композиция оставляет headroom). PNG ~1.6-2.5 MB, ~106 MB total — fe конвертит в WebP/AVIF + resize. Temp-скрипт удалён, ничего не закоммичено (constraint). **СЛЕДУЮЩЕЕ для po:** D3 wire-up — заменить CSS-gradient fallback на реальные пути `assets/services/vyvoz-musora/{hero,process,cases,districts,calculator}/*.png` в Payload Media; commit assets в репо.

## Где были раньше (2026-05-11 ~14:35 MSK) — C5 wave B real content на /vyvoz-musora/ + markdown-bug fixed

**cw+dev (autonomous mandate):** EPIC-SERVICE-PAGES-UX C5 wave B — реальный TOV-контент в pilot pillar.

- 2026-05-11 14:35 MSK · cw+dev → po: C5 wave B done (vyvoz-musora pilot). Контент написан вручную (Claude в задаче, БЕЗ ANTHROPIC_API_KEY), TOV brand-guide v2.6 §tov Caregiver+Ruler, конкретные цифры (12 800/14 200/14 800 ₽, 8/20/27 м³, 152-ФЗ, Росприроднадзор). **3 новых блока** на Service id=3 (direct SQL на prod, idempotent): calculator-placeholder с реальным текстом «Точный расчёт за 10 минут — пришлите три фото объекта» (вместо пустого блока / TODO-жаргона); pricing-table 3-tier Эконом/Стандарт/Премиум (12 800/14 200/14 800 ₽, badge «Выбирают чаще»); process-steps 6 шагов заявка→расчёт→подача контейнера→погрузка→вывоз→акт. **Markdown-bug fixed**: text-content body был Lexical-объектом с raw `## …` внутри одного paragraph node → перезаписан чистым markdown-string (TextContent renderer корректно рендерит `## ` → h3); раздел `## Как мы работаем` убран из текста — его покрывает process-steps. **Migration `20260511_120000_services_blocks_v26_extension`** (services_blocks_pricing_table/_process_steps + sub-tables + _services_v mirror) applied на prod (scp → `psql -v ON_ERROR_STOP=1 -f` → registered в payload_migrations → ownership→obikhod). Backup ДО: `Prod backup (pg_dump)` run #25667115074 SUCCESS. **Code changes в working tree (НЕ pushed — constraint):** `Services.ts` blockReferences +PricingTable +ProcessSteps; `getBlocksForLayer.ts` SLUG_TO_SECTION +`pricing-table`→`pricing-block` +`process-steps`→`process` aliases (sync с master-template.ts); 3 migration files; `scripts/fill-vyvoz-musora-content.{ts,sql}`. `pnpm type-check` 0 errors / lint 0 / prettier clean / `pnpm test:unit` 59/59 PASS. **`use_template_v2` = FALSE** для id=3 (важно): deployed resolver `getBlocksForLayer` НЕ знает `pricing-table`/`process-steps` aliases → при templateV2=true дропнул бы их и нарисовал placeholder'ы «Прайс-лист добавляется». При templateV2=false → identity passthrough, deployed BlockRenderer рендерит все 14 блоков как есть, 0 placeholders. **СЛЕДУЮЩЕЕ для po: commit+deploy code changes → потом `UPDATE services SET use_template_v2=true WHERE id=3` → получим master-template reorder + pricing/process visible.** Пока (до deploy) pricing-table/process-steps в DB но НЕ рендерятся (deployed Payload config не имеет их в blockReferences → afterRead их игнорит); видны: markdown-fixed text-content, real calculator copy, 0 placeholders, HTTP 200, H2 list реальный. **Pre-existing landmine (не моя зона):** 2 cta-banner блока на doc 3 (order 4+9) → master-template-gate бросит DUPLICATE_SECTION при re-save в admin (resolver dedup'ит). **Smoke**: HTTP 200, 0× `data-placeholder`, 0× raw `## `, 0× TODO-жаргон («по брифу»/«в работе»/«добавляется»/«sustained»), other pillars (arboristika/chistka-krysh/demontazh) HTTP 200.

## Где были раньше (2026-05-10 ~22:40 MSK) — C2.6 issue 3 Calculator enum fix applied prod
