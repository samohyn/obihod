...
| 3 | ✅ | `team/WORKFLOW.md` переписан под 7 команд + двухступенчатый релиз-цикл + sticky sessions §5.4 + branch strategy §6 + 6 Linear team keys; `team/PROJECT_CONTEXT.md` переписан; `CLAUDE.md` обновлён |
| 4 | ✅ | Linear: создано 27 labels в PRODUCT team (25 role + 2 phase: gate/verify); SHOP team — 30 labels (3 group + 9 phase + 3 segment + 15 role); PANEL team — 30 labels. Старые labels (po/out/fe1/2/be1..4/qa1/2/seo1/2/sa/cr) НЕ удалены — MCP не поддерживает delete_issue_label, оператор удалит вручную в UI после миграции active issues |
| 5 | ✅ | 5 memory файлов (sticky_agent_sessions, skill_check_iron_rule, release_cycle_v2, team_v2_42_roles, linear_release_mgr_alias) + MEMORY.md index; hooks audit чист |
| 6 | ✅ | ADR-0004-team-restructure-v2.md в `team/adr/`; verification-suite пройден |
| 7 | ✅ | **Spec-before-code iron rule** добавлен в 22 файла (3 sa + 16 dev + 3 PO в product/shop/panel) + WORKFLOW.md §3 фаза 7 + §4 RACI; memory `feedback_spec_first_iron_rule.md` |
...
**Spec-before-code iron rule (Phase 7, 2026-04-28):**
В командах panel/product/shop dev/qa/cr НЕ стартует без одобренной `sa-<team>.md` спеки. PO команды держит gate в DoD. Iron rule добавлен в 22 файла + WORKFLOW.md.

**Linear teams и labels (Phase 4 закрыта 2026-04-28):**
- **PRODUCT team** (`OBI`) — 27 новых labels (25 role + 2 phase). Старые role labels (po/out/fe1/2/be1..4/qa1/2/seo1/2/sa/cr) остались как aliases.
- **SHOP team** (новая, создал оператор) — 30 labels: 3 group (phase/role/segment) + 9 phase + 3 segment + 15 role (7 native shop + 8 cross-team).
- **PANEL team** (новая, создал оператор) — 30 labels: аналогичная структура.
- **Прочие existing teams** (Design, SEO, Dev) — labels не трогал, они рабочие.

**Что НЕ сделано (по дизайну, для следующих сессий):**
- **Содержимое подкомандных ролей** (be-site, fe-shop, qa-panel, sa-shop, и т.д.) — frontmatter и iron rules обновлены, но текстовая часть частично от копий старых файлов. Каждая роль дополнит свою спеку при первой задаче в её зоне.
- **Старые Linear role labels** (po, out, fe1/2, be1..4, qa1/2, seo1/2, sa, cr в PRODUCT team) — оставил как aliases, MCP не поддерживает delete_issue_label. Оператор удалит вручную в UI после миграции active issues.
- **Active specs `team/specs/US-*/`** — ссылки на старые codes пока не переписаны. Когда такая задача станет активной (например, при возврате к US-8/9/10) — заменить на новые codes.
- **`epic` group label** в SHOP/PANEL — пока пусто, появится с первым эпиком (`epic/us-N`).

**Что в репо:**
- Ветка `chore/team-restructure-v2` смержена в `fix/admin-stat-cards-12-3` (`--no-ff`, commit `17ff224`).
- Phase 7 — отдельный коммит `0e8f86a` на `fix/admin-stat-cards-12-3`.
- Запушено на github (`origin/fix/admin-stat-cards-12-3`).
