...
- **Запуск SEO/analytics на prod (оператор):**
  - `git push origin main` — триггерит build+deploy, Метрика сразу в бандле
  - После деплоя: smoke `curl https://obikhod.ru/ | grep mc.yandex.ru` + в Метрике убедиться что hits идут
  - В Вебмастере: Индексирование → Файлы Sitemap → `https://obikhod.ru/sitemap.xml`
  - В Вебмастере: связать со счётчиком Метрики 108715562
  - Завести цели в Метрике: `lead_sent`, `photo_upload`, `phone_click`, `tg_click`, `max_click`, `wa_click`, `calc_submit` — привяжет к UI позже [fe1]/[fe2]/[aemd]
  - Google Search Console — параллельно, если нужен (добавлю `verification.google` по запросу)
- **Pre-launch blocker pack (2 US, параллельно):**
  - **US-1-seed-prod-db** — intake готов, ждёт apr оператора на 4 открытых вопроса → `ba`
  - **US-2-cms-header-footer-globals** — intake + все 8 ответов оператора
    зафиксированы (2026-04-22). Scope утверждён. Передано в `ba` на
    бизнес-декомпозицию. Ключевые решения: 1 global, телефон `+7 (985) 170-51-11`
    финальный, соцсети массивом {type,url}, меню union (anchor/route/external),
    ИНН временный `7847729123`, баннер и region-switcher — в бэклог,
    копирайт с автогодом. Подробнее — `devteam/specs/US-2-cms-header-footer-globals/intake.md`
  - оба — P0, публичный запуск не состоится без обоих
