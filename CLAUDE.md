...
7. **PO orchestration — самостоятельная передача между фазами** — PO команды
   (`podev` / `poseo` / `popanel` / `poshop` / `cpo`) **обязан и имеет право
   самостоятельно** подключать компетентных агентов под контекст задачи и
   переключать фазы артефакта без эскалации к оператору:
   ...
   - нужен другой skill (`tamd` для архитектуры, `dba` для миграций, `aemd`
     для разметки, `do` для CI) — подключает напрямую через Hand-off log в
     `specs/<EPIC|TASK>/<US>/`.

   **Если PO сомневается** (cross-team конфликт ресурсов, неясный owner, риск
   нарушить scope другой команды, сомнение в выборе skill/роли) — **спрашивает
   оператора** до переключения фазы, не делает молча. Каждый hand-off
   фиксируется строкой в Hand-off log артефакта. Оператор не bottleneck для
   штатных переходов между фазами одной команды.
